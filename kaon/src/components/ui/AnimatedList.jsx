...
const AnimatedList = ({
  ...
  return (
    <div className={`relative w-[500px] ${className}`}>
      <div
        ref={listRef}
        className={`max-h-[100vh] overflow-y-auto p-4 ${
          displayScrollbar
      ...
      >
        {items.map((item, index) => (
          ...
          >
            <div
              className={`p-4 bg-[#111] rounded-lg transition-all duration-200 ${
                selectedIndex === index ? 'bg-[#222] scale-[1.02]' : 'hover:bg-[#191919]'
              } ${itemClassName}`}
            >
              {/* ✅ Tambahkan gambar di atas judul */}
              {item.image && (
                <img
                  src={item.image}
